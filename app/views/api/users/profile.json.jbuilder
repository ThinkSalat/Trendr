json.user do 
  json.extract! @user, 
  :id, 
  :username, 
  :email, 
  :title, 
  :description, 
  :private_posts, 
  :private_likes, 
  :private_followers, 
  :private_followings
  json.avatar @user.avatar.service_url
  json.followed_users do
     json.array! @user.followed_users.pluck(:id)
  end
end

json.posts do
  @user.posts.where("posts.created_at <= :date", {date: @date}).limit(5).offset(@offset).each do |post|
    json.set! post.id do
      json.extract! post, :id, :user_id, :title, :slug, :state, :post_type, :summary, :body, :private, :photoset_layout, :caption, :source_url, :source_title, :created_at
      json.images do 
        json.array! post.images do |img| 
          json.url img.service_url 
        end
      end
      json.likes do
        json.array! post.users_who_like_post_ids
      end
      json.number_notes post.likes.count
    end
  end
end